...
class DynamicProgramming:
    ...
    def find_index_of_new_state(self, news):
        for ind, s in enumerates(self.states):
    ...
    def main(self):
        ...
        while True:
            ...
            for ind, s in enumerate(self.states):
               # list with q_value depending on s and a with our model impossible 2 have 2 states from one state and one action
                q_s_a = [0]*len(self.actions)

               #for each action
                for ind_a, a in self.actions:
                    r_s_a,p_sPrime_knowingSandA, s_prime = sim.launch(s, a, 1)
                    ind_s_prime = self.find_index_of_new_state(s_prime)
                    if ind_s_prime==None:
                        break
                    q_s_a[ind_a] = r_s_a+self.discounted_factor*(p_sPrime_knowingSandA*v_value_prime[ind_s_prime])
